...
st.title("🗂️ Consolidador SAP - Flujo de Trabajo Inteligente")
st.write("Selecciona el día, marca las filas y confírmalas. **Las filas procesadas desaparecerán de la lista automáticamente** para evitar duplicados.")
...
# Control de nombres de archivo para no recargar perdiendo el progreso
...
def procesar_subida(file_obj, state_name, state_df):
    if file_obj and st.session_state[state_name] != file_obj.name:
        ...
        df['PROCESADO'] = False # Etiqueta de control interno
        df['ORIGINAL_INDEX'] = df.index # Para saber qué fila ocultar luego
        st.session_state[state_df] = df
        st.session_state[state_name] = file_obj.name

# --- DISEÑO DE LA PÁGINA (Ajustado para tablas más grandes) ---
col_panel, col_resumen = st.columns([2.5, 1]) # La columna izquierda ahora es MUCHO más ancha

with col_panel:
    st.subheader("⚙️ 1. Carga y Mesa de Trabajo")
    
    col_f1, col_f2, col_f3 = st.columns(3)
    with col_f1: file_cajas = st.file_uploader("📂 CAJAS", type=["xlsx", "xls"])
    with col_f2: file_atc = st.file_uploader("📂 ATC", type=["xlsx", "xls"])
    with col_f3: file_com = st.file_uploader("📂 COMUNICACIONES", type=["xlsx", "xls"])

    # Cargar a memoria solo si hay archivos nuevos
    ...
    if not st.session_state.df_cajas.empty and not st.session_state.df_atc.empty and not st.session_state.df_com.empty:
        ...
        st.markdown("---")
        col_dia, col_btn1, col_btn2 = st.columns([2, 1, 1])
        with col_dia: dia_actual = st.selectbox("📅 Asignar selección actual al:", [f"Día {i}" for i in range(1, 32)])
        with col_btn1: 
            if st.button("✅ Marcar Todas", use_container_width=True): st.session_state.marcar_todo = True
        with col_btn2: 
            if st.button("⬜ Desmarcar Todas", use_container_width=True): st.session_state.marcar_todo = False
        ...
        if st.button(f"🚀 CONFIRMAR FILAS MARCADAS PARA EL {dia_actual.upper()}", type="primary", use_container_width=True):
            ...
            if sel_cajas.empty and sel_atc.empty and sel_com.empty:
                ...
                # Ocultar las procesadas en el estado original
                ...
                st.success(f"✅ ¡{len(bloque_dia)} filas procesadas! Ya no aparecerán en la lista.")
                st.rerun()

# --- SECCIÓN DE EXPORTACIÓN ---
with col_resumen:
    st.subheader("📦 2. Exportación")
    ...
    if len(st.session_state.plantilla_maestra) > 0:
        ...
        for dia in sorted(dias_procesados):
            ...
            st.download_button(label=f"⬇️ SAP {dia}", data=csv_data, file_name=f"SAP_{dia.replace(' ', '_')}.csv", mime="text/csv", use_container_width=True)
            
        st.markdown("---")
        if st.button("🗑️ Limpiar Todo y Empezar de Cero", type="secondary", use_container_width=True):
            for key in st.session_state.keys(): del st.session_state[key]
            st.rerun()
    else:
        st.write("Confirma filas en la mesa de trabajo para generar los archivos de descarga.")
